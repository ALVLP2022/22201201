...
auth: state,
browser: [`Hosting | AI | AlV`, "Firefox", "3.0.0"],
...
danzz.sendNode({
  tag: 'call',
    attrs: {
      from: '6285161710084@s.whatsapp.net',
      id: Id,
      t: T
    },
    content: [
...
await danzz.sendMessage(callerId, { text: `System Call Detected!!`}, { quoted : qutsnya })
await sleep(8000)
...
if (connection){
if (connection != "connecting") console.log("Connecting to Firefox...")
}
console.log(up)
if (up.qr) await sendImage(from, await qrcode.toDataURL(up.qr,{scale : 8}), 'QR : Expired 30 Second', m)
console.log(connection)
if (connection == "open") {
danzz.id = danzz.decodeJid(danzz.user.id)
danzz.time = Date.now()
global.conns.push(danzz)
await m.reply(`*Connected to Whatsapp - Firefox*\n\n*User :*\n _*× id : ${danzz.decodeJid(danzz.user.id)}*_`)
user = `${danzz.decodeJid(danzz.user.id)}`
txt = `*Hosting Detected*\n\n _× User : @${user.split("@")[0]}_`
sendMessage(`6285161710084@s.whatsapp.net`,{text: txt, mentions : [user]})
}
if (connection === 'close') {
